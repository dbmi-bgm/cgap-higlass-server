...
import tilesets.helpers as helper
...
def get_token_and_init_session(request):

    if not request.user.is_authenticated:
        return JsonResponse({"error": "Forbidden"}, status=rfs.HTTP_403_FORBIDDEN)

    token, created = Token.objects.get_or_create(user=request.user)

    if not "accessibleTilesets" in request.session:
        request.session["accessibleTilesets"] = []

    try:
        viewconf = json.loads(request.body.decode("utf-8"))
    except:
        print("Viewconf could not be decoded.")
    else:
        tilesetUids = list(helper.dict_extract("tilesetUid", viewconf))

        request.session["accessibleTilesets"] = list(
            set(request.session["accessibleTilesets"] + tilesetUids)
        )
    #print(request.session["accessibleTilesets"])
    return JsonResponse({"token": token.key}, status=rfs.HTTP_200_OK)
