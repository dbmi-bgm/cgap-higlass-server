...
def tiles(request):
    ...
    accessible_tilesets = [(t, tileids_by_tileset[t.uuid], raw, tileset_to_options.get(t.uuid, None)) for t in tilesets if ( ((not t.private) or request.user == t.owner) and (not t.requiresAuthentication or request.user.is_authenticated) )]
...
def tileset_info(request):
    ...
    for tileset_uuid in tileset_uuids:
        ...
        if tileset_object.requiresAuthentication and not request.user.is_authenticated:
            # dataset is not public
            tileset_infos[tileset_uuid] = {'error': "You don't have access to this tileset"}
            continue
